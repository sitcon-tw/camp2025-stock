...
const CandlestickChart = ({
    ...
    const actualWidth = isMobile
        ? Math.min(450, window.innerWidth - 20)
        : width;
            ...
            >
                ...
                >
                    <g className="grid-lines">
                        {(isMobile ? [0.5] : [0.25, 0.75]).map(
                            (ratio) => {
                                const y = yScale(
                                    minPrice + priceRange * ratio,
                                );
                                return (
                                    <line
                                        key={ratio}
                    ...
                    >
                        {data.map((d, i) => {
                            ...
                            const bodyTop = yScale(
                                Math.max(open, close),
                            );
                            const bodyBottom = yScale(
                                Math.min(open, close),
                            );
