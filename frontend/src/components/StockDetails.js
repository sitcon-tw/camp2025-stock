    ...
    <div className="bg-[#1a2e4a] rounded-lg p-4 border border-[#82bee2]/10">
      ...
      <div className="flex flex-col">
        <span className={`text-lg font-bold ${
          isChange ? (isPositive ? 'text-red-400' : 'text-green-400') : 'text-white'
        }`}>
          {isPrice ? '$' : ''}{value}
        </span>
        {subValue && (
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <DetailCard 
            ...
            subValue={`${isPositive ? '+' : ''}$${changeAmount.toFixed(2)}`}
            isChange={true}
          />
          <DetailCard 
            title="開盤價" 
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <DetailCard 
            title="52週最高" 
            value={weekHigh52.toFixed(2)} 
            isPrice={true}
          />
          <DetailCard 
            title="52週最低" 
            value={weekLow52.toFixed(2)} 
            isPrice={true}
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          ...
          <DetailCard 
            title="60日均線" 
            value={(currentPrice * 0.88).toFixed(2)}
            isPrice={true}
          />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
